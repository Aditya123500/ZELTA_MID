...
Original file is located at
    https://colab.research.google.com/drive/1aSOwmKv4NGtf5ye3voVW0XiT0ZiL81gw
"""
...
import os
os.chdir("/content/drive/MyDrive/ZELTA_INTER")
...
def process_data(filepath_4h,filepath_1D):
    ...
    def process_btc1_data(csv_file_path):
        ...
        def strat(data):
            ...
            #data = data[data['datetime'] <= '2021-11-10 00:00:00']
        ...
        #test['signals']= test['signals']+ test['signals_new']
    ...
    def process_btc3_data(filepath):
        ...
        #df = df[df['datetime'] <= '2021-11-10 00:00:00']
        ...
        df['signals']= df['signals']+ df['signals_new']
...
def strat(start1,strat2,strat3,strat4):
    ...
    master.to_csv()
    btc_4h = pd.read_csv("BTC_2019_2023_4h.csv")
    btc_4h.index = btc_4h['datetime']
    btc_4h = btc_4h['2020-01-01':]
    master.index = btc_4h['datetime']
    btc_4h['signals'] = master['signals'].fillna(0).astype(int)
    sig_to_trade(btc_4h)
...
if __name__ == "__main__":
    ...
    strat1,strat2,strat3,strat4 = process_data("BTC_2019_2023_4h.csv",'BTC_2019_2023_1d.csv')
    last_key= strat(strat1,strat2,strat3,strat4)
...
last_key['datetime'] = pd.to_datetime(last_key['datetime'])
last_key = last_key.set_index('datetime')
last_key

def add_stop_loss(data):
            data['SL'] = 0.0  # Initialize the 'SL' column
            for index, row in data.iterrows():
                if row['signals'] == 1 or 2:
                    data.loc[index, 'SL'] = row['close'] * 0.85
                elif row['signals'] == -1 or -2:
                    data.loc[index, 'SL'] = row['close'] * 1.15
            return data
...
last_key= add_stop_loss(last_key)
last_key = add_take_profit(last_key)

# prompt: convert final_key to csv

last_key.to_csv('final_key.csv', encoding = 'utf-8-sig')

# prompt: downlapd last_key

from google.colab import files
files.download('final_key.csv')
...
if __name__ == "__main__":
  ...
  # csv_file_path = "/content/untrade-sdk/examples/example_csv.csv"

  # Perform backtest using the provided CSV file path
  backtest_result = perform_backtest('final_key.csv')
